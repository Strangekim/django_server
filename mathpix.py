...
1. 'problem' (필수, 문자열 타입)
   [중요] problem 추출 규칙:
   - 문제 번호(1번, 2번 등)와 [점수], [배점] 같은 메타정보는 제거하세요.
   - 문제는 질문이 끝나는 지점까지만 포함하세요.
   - 질문 끝 표시: "~구하시오.", "~값은?", "~개수는?", "~얼마인가?" 등으로 끝남
   - 보기(선택지)가 시작되기 전까지만 포함하세요.
   - 보기 앞의 불필요한 문자(+, -, 공백 라인 등)는 모두 제거하세요.
   - 예시:
     * "g(−2) + g(2)의 값은?" ← 여기까지가 문제
     * 이후에 나오는 "+ (1) 30 (2) 32..." ← 이 부분은 problem에 포함하지 않음
   - 만약 문제 본문을 찾을 수 없다면, 빈 문자열("")이 아닌 "문제 내용을 추출할 수 없습니다"를 반환하세요.
...
2. 'choices' (필수, 문자열 배열 타입)
   ...
   - 보기의 실제 내용만 추출하세요.
   - 분수는 LaTeX가 아닌 일반 문자로: 3/8, 1/2, 5/8 등
   - 숫자는 그냥 숫자로: 22, 24, 26 등
   - 예시:
     * 문제에 "① 3/8  ② 1/2  ③ 5/8  ④ 3/4  ⑤ 7/8" 이렇게 있으면
       choices는 ["3/8", "1/2", "5/8", "3/4", "7/8"]
     * 문제에 "(1) 30  (2) 32  (3) 34  (4) 36  (5) 38" 이렇게 있으면
       choices는 ["30", "32", "34", "36", "38"]
     * 문제에 "① x+2  ② x-2  ③ 2x  ④ x/2" 이렇게 있으면
       choices는 ["x+2", "x-2", "2x", "x/2"]
...
4. 'solution_steps' (필수, 객체 배열 타입)
   ...
   - 예: [{"step_number": 1, "description": "구의 부피 공식을 세운다"}, {"step_number": 2, "description": "원기둥의 부피와 비교한다"}]
